...
uint8_t counter_get(counter_t counter, uint8_t face);
uint8_t counter_get(counter_t counter, uint8_t face) {
    assert(1 <= face);
    assert(face <= 6);
...
uint8_t counter_get_face_by_count(counter_t, uint8_t target_count);
uint8_t counter_get_face_by_count(counter_t counter, uint8_t target_count) {
    for (size_t i = 0; i < 6; ++i)
        if (counter.counts[i] == target_count) return i + 1;
    return 0;
}

uint8_t counter_get_face_at_least(counter_t, uint8_t target_count);
uint8_t counter_get_face_at_least(counter_t counter, uint8_t target_count) {
    for (size_t i = 0; i < 6; ++i)
        if (counter.counts[i] >= target_count) return i + 1;
    return 0;
}
...
bool counter_is_little_straight(counter_t counter) {
    uint8_t const little_straight[] = {1, 1, 1, 1, 1, 0};
...
bool counter_is_big_straight(counter_t counter) {
    uint8_t const big_straight[] = {0, 1, 1, 1, 1, 1};
