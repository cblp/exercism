...
def check_drinks(drink_name: str, drink_ingredients: list[str]) -> str:
    ...
    return drink_name + (
        " Cocktail" if ALCOHOLS & set(drink_ingredients) else " Mocktail"
    )
...
def singleton_ingredients(dishes: list[set[str]], intersection: set[str]) -> set[str]:
    ...
    Each `<CATEGORY>_INTERSECTIONS` is an `intersection` of all dishes
    in the category. `<CATEGORY>` can be any one of:
