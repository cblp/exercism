...
Inventory = dict[str, tuple[int, str, bool]]

InventoryOut = dict[str, tuple[int | str, str, bool]]


def send_to_store(cart: Cart, aisle_mapping: dict[str, tuple[str, bool]]) -> Inventory:
...
def update_store_inventory(
    ...
    return {
        item: (
            (
                count - (fulfillment_cart[item][0] if item in fulfillment_cart else 0)
                or "Out of Stock"
            ),
            aisle,
