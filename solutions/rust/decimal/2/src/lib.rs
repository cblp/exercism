...
impl Decimal {
    ...
    fn unify(self, b: Self) -> (BigInt, BigInt, i64) {
        match self.exponent.cmp(&b.exponent) {
            Ordering::Equal => (self.significand, b.significand, self.exponent),
            Ordering::Less => (
                self.significand,
                b.significand
                    * BigInt::from(10)
                        .pow((b.exponent - self.exponent).try_into().unwrap()),
                self.exponent,
            ),
            ...
            Ordering::Greater => (
                self.significand
                    * BigInt::from(10)
                        .pow((self.exponent - b.exponent).try_into().unwrap()),
                b.significand,
                b.exponent,
            ),
        }
...
impl Add for Decimal {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        let (self_sig, rhs_sig, exponent) = self.unify(rhs);
...
impl Ord for Decimal {
    fn cmp(&self, other: &Self) -> Ordering {
        let (self_sig, other_sig, _) = self.clone().unify(other.clone());
